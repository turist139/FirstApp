...
struct TimeRemainingProvider: TimelineProvider {
    ...
    private func fetchLatestData() -> TimeEntry {
        ...
        guard let container = try? ModelContainer(for: schema, configurations: [config]) else {
            let now = Date()
            let endOfCurrentDay = DetoxDateHelper.endOfDetoxDay(for: now, boundaryHour: boundaryHour)
        ...
        let now = Date()
        let endOfCurrentDay = DetoxDateHelper.endOfDetoxDay(for: now, boundaryHour: finalBoundary)
